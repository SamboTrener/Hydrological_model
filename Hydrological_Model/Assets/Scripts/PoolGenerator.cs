...
public class PoolGenerator : MonoBehaviour
{
    ...
    void CreatePool(float[,] poolMap, float[,] map, WaterDroplet droplet, int mapSize)
    {
        ...
        var pool = new Pool(points, initialPoolVolume, id);  //—оздан бассейн в одной точке 
        id++;
    ...
    {
    Again:
        FindLowestPointOfLeak(map, poolMap, pool, mapSize); //“очка утечки нова€ или равна уровню воды
        ...
        while (pool.LowestPointOfLeak.Height + epsilon < poolMap[testingPoint.Y, testingPoint.X])
        {
            ...
            if (poolMap[pool.LowestPointOfLeak.Y, pool.LowestPointOfLeak.X] > map[pool.LowestPointOfLeak.Y, pool.LowestPointOfLeak.X]) //¬ точке базируетс€ другой бассейн
            {
            ...
            testingPoint = pool.Points.First();  //ќбновл€ем точку тестировани€, потому что стара€ могла выйти из бассейна
            ...
            for (int y = -1; y <= 1; y++)
            {
                ...
                for (int x = -1; x <= 1; x++)
                {
                    ...
                    if (pool.Points.FirstOrDefault(poolPoint => poolPoint.Y == point.Y + y && poolPoint.X == point.X + x) == null) 
                        //≈сли соседн€€ точка не в бассейне
                    {
                        if (poolMap[point.Y, point.X] > map[point.Y + y, point.X + x]) 
                            //Ќайдена более низка€ точка относительно уровн€
                        {
    ...
    {
        var testingPoint = pool.Points.First(); //”ровень воды в бассейне определ€етс€ любой точкой бассейна

        var dif = poolMap[testingPoint.Y, testingPoint.X] - map[pointToAdd.Y, pointToAdd.X];

        pool.Points.Add(pointToAdd); //“очка стала частью бассейна
        poolMap[pointToAdd.Y, pointToAdd.X] = map[pointToAdd.Y, pointToAdd.X] + dif; //ѕосчитали в точке уровень воды

        foreach (var point in pool.Points) //ѕересчитали уровень воды дл€ других точек
        {
    ...
    {
        foreach (var point in secondPool.Points) //poolMap занул€ем, будем добавл€ть точки с нул€ 
        {
            poolMap[point.Y, point.X] = 0;
        }
        AddVolumeEqually(poolMap, pool, secondPool.Volume); //–азмазали объЄм второго бассейна по первому
        pool.Volume += secondPool.Volume;

        foreach (var pointToAdd in secondPool.Points) //ƒобавл€ем все точки второго бассейна в первый
        {
            AddPointInPool(map, poolMap, pool, pointToAdd);
        }

        while (!IsPoolCorrect(map, poolMap, pool)) // оррекци€
        {
            CorrectPoolOnce(map, poolMap, pool);
        }
